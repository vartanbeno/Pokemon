...
/**
 * 
 * Provides methods to find records in the CardInPlay table.
 * 
 * @author vartanbeno
 *
 */
public class CardInPlayFinder {

	private static final String FIND_ALL = String.format("SELECT %1$s FROM %2$s;", CardInPlayTDG.getColumns(), CardInPlayTDG.getTableName());
	
	private static final String FIND_BY_ID = String.format("SELECT %1$s FROM %2$s "
			+ "WHERE id = ?;", CardInPlayTDG.getColumns(), CardInPlayTDG.getTableName());
	
	private static final String FIND_BY_CARD = String.format("SELECT %1$s FROM %2$s "
			+ "WHERE card = ?;", CardInPlayTDG.getColumns(), CardInPlayTDG.getTableName());
	
	private static final String FIND_BY_GAME_AND_PLAYER = String.format("SELECT %1$s FROM %2$s "
			+ "WHERE game = ? AND player = ?;", CardInPlayTDG.getColumns(), CardInPlayTDG.getTableName());
	
	private static final String FIND_BY_GAME_AND_PLAYER_AND_STATUS = String.format("SELECT %1$s FROM %2$s "
			+ "WHERE game = ? AND player = ? AND status = ?;", CardInPlayTDG.getColumns(), CardInPlayTDG.getTableName());
